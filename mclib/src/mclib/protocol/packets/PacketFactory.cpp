...
Packet* PacketFactory::CreatePacket(Protocol& protocol, protocol::State state, DataBuffer data, std::size_t length, core::Connection* connection) {
    ...
    // DEBUG
    // Logger::log("Rec - " + std::hex + vid.GetInt() + std::dec + Colors::End);
    ...
    if (packet) {
        ...
        // DEBUG
        /*if (status == 0) {
            Logger::log("Received Packet - " + demangled_name +  Colors::End);
        } else {
            Logger::log("Received Packet - " + typeid(*packet).name() + Colors::End);
        }*/
